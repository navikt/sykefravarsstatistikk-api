...
@DataJdbcTest
class AlleNaring5SifferRepositoryTest {

    @Autowired
    private NamedParameterJdbcTemplate jdbcTemplate;

    private AlleNaring5SifferRepository alleNæring5SifferRepository;

    @BeforeEach
    void setUp() {
        slettAllStatistikkFraDatabase(jdbcTemplate);
        alleNæring5SifferRepository = new AlleNaring5SifferRepository(jdbcTemplate);
        Næring produksjonAvKlær = new Næring("14190", "Produksjon av klær");
        ...
        jdbcTemplate.update(
                "insert into sykefravar_statistikk_naring5siffer (naring_kode, arstall, kvartal, antall_personer, tapte_dagsverk, mulige_dagsverk) "
                        + "VALUES (:naring_kode, :arstall, :kvartal, :antall_personer, :tapte_dagsverk, :mulige_dagsverk)",
                parametre(new Næring("86907", "Undervisning"), 2019, 2, 10, new BigDecimal(5), new BigDecimal(100))
        );
        jdbcTemplate.update(
                "insert into sykefravar_statistikk_naring5siffer (naring_kode, arstall, kvartal, antall_personer, tapte_dagsverk, mulige_dagsverk) "
                        + "VALUES (:naring_kode, :arstall, :kvartal, :antall_personer, :tapte_dagsverk, :mulige_dagsverk)",
                parametre(new Næring("86907", "Undervisning"), 2018, 4, 10, new BigDecimal("7.5"), new BigDecimal(100))
        );
    ...
    void hentSykefraværprosentAlleNæringerForEttKvartal__skalMaskereVirksomheterUnderMinimumAntallPersoner() {
        ...
        assertTrue(resultat.stream().anyMatch(
                sykefraværsstatistikkNæring ->
                        sykefraværsstatistikkNæring.getTapteDagsverk().compareTo(new BigDecimal(2)) == 0 && sykefraværsstatistikkNæring.getNæringkode().equals("14190")
        ));
