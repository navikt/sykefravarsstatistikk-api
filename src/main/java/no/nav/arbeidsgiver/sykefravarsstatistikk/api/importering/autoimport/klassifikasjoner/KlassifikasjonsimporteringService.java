...
public class KlassifikasjonsimporteringService {
    ...
    private final KlassifikasjonsimporteringRepository
    ...
    public OpprettEllerOppdaterResultat populerSektorer() {
        ...
        OpprettEllerOppdaterResultat resultat = opprettEllerOppdaterVirksomhetsklassifikasjoner(
                sektorer,
                Klassifikasjonskilde.SEKTOR
        );
        log.info(
                String.format(
    ...
    private OpprettEllerOppdaterResultat opprettEllerOppdater(
        ...
        klassifikasjonsimporteringRepository.hent(virksomhetsklassifikasjon, klassifikasjonskilde).ifPresentOrElse(
                eksisterendeKlassifikasjon -> {
                    if (!eksisterendeKlassifikasjon.equals(virksomhetsklassifikasjon)) {
                        klassifikasjonsimporteringRepository.oppdater(virksomhetsklassifikasjon, klassifikasjonskilde);
                        resultat.setAntallRadOppdatert(1);
                    }
                },
                () -> {
                    klassifikasjonsimporteringRepository.opprett(virksomhetsklassifikasjon, klassifikasjonskilde);
